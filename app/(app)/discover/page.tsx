...
import { businesses, industries, vendorsAndDistributors } from '@/lib/mock-data'

const topVendors = [...vendorsAndDistributors].sort(
  (a, b) => (b.trust?.score ?? 0) - (a.trust?.score ?? 0),
)
...
export default function DiscoverPage() {
  ...
  const isBrowsing = !query && industry === 'All'
    ...
    <div className="pb-8">
      <header className="sticky top-0 z-20 border-b border-border bg-background/85 px-4 py-3 backdrop-blur">
        <div className="flex items-center justify-between">
          <h1 className="font-display text-lg font-bold tracking-tight">{t('discover.title')}</h1>
          <button
            aria-label="Filters"
      ...
      <div className="px-4">
        <SectionTitle
          title={query || industry !== 'All' ? `${results.length} result${results.length === 1 ? '' : 's'}` : t('discover.allBusinesses')}
        />
        {results.length === 0 ? (
          <EmptyState
